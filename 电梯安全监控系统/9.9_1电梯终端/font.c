...
#define getB(c) ((c>> 8)&0x000000ff)
#define getG(c) ((c>>16)&0x000000ff)
#define getR(c) ((c>>24)&0x000000ff)
...
void setPixel(bitmap *bm, u32 x, u32 y, color c){
	if(bm->byteperpixel == 3){
		...
		*(bm->map + y*bm->width*bm->byteperpixel + x*bm->byteperpixel + 0) = getB(c);
		*(bm->map + y*bm->width*bm->byteperpixel + x*bm->byteperpixel + 1) = getG(c);
		*(bm->map + y*bm->width*bm->byteperpixel + x*bm->byteperpixel + 2) = getR(c);
		...
		for(iy=0; iy<charHeight; iy++){
			for(ix=0; ix<charWidth; ix++){
				...
				if(charRaster[ix+iy*charWidth]!=0 && xpos<screen->width && ypos<screen->height){
					...
					u8 r = (alpha * getB(c) + invAlpha * bgr) >> 8;
					u8 g = (alpha * getG(c) + invAlpha * bgg) >> 8;
					u8 b = (alpha * getR(c) + invAlpha * bgb) >> 8;
...
//w：汉字显示区域宽，h：汉字显示区域高，x，y：从bmp图片的x，y开始获取
void setfontPix(bitmap *font,struct Bmp* mp,int w,int h,int x,int y,unsigned int col)
{
	unsigned int data=0;
     for (int i = 0; i < h; i++)//每循环一边就初始化一下背景，固定背景
            {
                for (int j = 0; j < w; j++)
                {
                    // color c = getPixel(font, 0 + j, 0 + i);
                    if(mp==NULL)
					{
                       data =  col;
                    }
					...
					{
					   data=getpix(mp, j+x, i+y);//获取图片像素
					}

					
                    setPixel(font, 0 + j, 0 + i, data); //将图片像素填充到文字背景
                    // printf("%d-%d\n",i,j);
...
void show_font(LcdDevice *lcd, int x, int y, bitmap *mp)
{
    ...
    for (int i = 0; i < dh; i++)
    {
        for (int j = 0; j < dw; j++)
        {
            ...
            memcpy(p + j, px + j * mp->byteperpixel, mp->byteperpixel);
        }
